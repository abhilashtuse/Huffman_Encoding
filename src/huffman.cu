...
void buildHuffmanTree(string text)
{
    ...
    char *d_input_string = NULL;
    err = cudaMalloc((void**)&d_input_string, str.length()* sizeof(char));
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to allocate device input string (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    }
    cudaMemset(d_input_string, '\0', str.length());
    err = cudaMemcpy(d_input_string, h_input_string, str.length() * sizeof(char),cudaMemcpyHostToDevice);
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to copy input string from host to device (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    ...
    err = cudaMalloc((void**)&d_final_str, text.length()* sizeof(char));
    if (err != cudaSuccess)
    {
        fprintf(stderr, "Failed to allocate device input string (error code %s)!\n", cudaGetErrorString(err));
        exit(EXIT_FAILURE);
    ...
    err = cudaMemcpy(h_final_str, d_final_str, text.length(), cudaMemcpyDeviceToHost);
    if (err != cudaSuccess)
    {
